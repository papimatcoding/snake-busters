...
function clearMoveKeys() { for (const code of ['KeyW','KeyA','KeyS','KeyD']) dispatchKey(code, false); }
function setMoveVector(x, y) {
  const dead = .24;
  dispatchKey('KeyA', x < -dead); dispatchKey('KeyD', x > dead);
  dispatchKey('KeyW', y < -dead); dispatchKey('KeyS', y > dead);
}
function canvasPointFromVector(x, y, magnitude = 1) {
  const rect = canvas.getBoundingClientRect();
  const radius = Math.min(rect.width, rect.height) * (.34 + .28 * magnitude);
  return { x: rect.left + rect.width / 2 + x * radius, y: rect.top + rect.height / 2 + y * radius };
}
function sendAim(x, y, magnitude = 1) {
  const p = canvasPointFromVector(x, y, magnitude);
  canvas.dispatchEvent(new PointerEvent('pointermove', { bubbles:true, pointerId:8100, pointerType:'mouse', clientX:p.x, clientY:p.y }));
  return p;
}
function startFire(x, y, magnitude = 1) {
  ...
  activeFire = true;
  canvas.dispatchEvent(new PointerEvent('pointerdown', { bubbles:true, pointerId:8200, pointerType:'mouse', button:0, clientX:p.x, clientY:p.y }));
}
function stopFire() {
  if (!activeFire) return;
  activeFire = false;
  const rect = canvas.getBoundingClientRect();
  canvas.dispatchEvent(new PointerEvent('pointerup', { bubbles:true, pointerId:8200, pointerType:'mouse', button:0, clientX:rect.left+rect.width/2, clientY:rect.top+rect.height/2 }));
}
...
function bindStick(root, { onMove, onRelease, label, icon, fireWhileHeld = false, preserve = false }) {
  if (!root) return;
  ...
  root.addEventListener('pointerdown', event => {
    ...
    update(event); if (fireWhileHeld) startFire(vector.x,vector.y,vector.magnitude);
  }, { capture:true });
  root.addEventListener('pointermove', event => {
    if (event.pointerId !== pointerId) return;
    event.preventDefault(); update(event); if (fireWhileHeld) sendAim(vector.x,vector.y,vector.magnitude);
  }, { capture:true });
  const release = event => {
    if (pointerId === null || (event.pointerId != null && event.pointerId !== pointerId)) return;
    event.preventDefault?.(); event.stopPropagation?.();
    const released = {...vector}; pointerId = null;
    thumb.style.transform = 'translate(0px, 0px)'; root.classList.remove('stick-active');
    if (fireWhileHeld) stopFire();
    onRelease?.(released.x,released.y,released.magnitude,event);
  };
  for (const type of ['pointerup','pointercancel','lostpointercapture']) root.addEventListener(type, release, { capture:true });
}
...
if (movePad) {
  ...
  let movePointer = null; const thumb = moveStick.querySelector('.stick-thumb');
  moveStick.addEventListener('pointerdown', event => { if (!isPortraitMobile()) return; event.preventDefault(); movePointer=event.pointerId; try{moveStick.setPointerCapture(movePointer)}catch{} });
  moveStick.addEventListener('pointermove', event => {
    if (event.pointerId !== movePointer) return;
    const rect=moveStick.getBoundingClientRect(), cx=rect.left+rect.width/2, cy=rect.top+rect.height/2;
    const max=Math.max(28,Math.min(rect.width,rect.height)*.31); let dx=event.clientX-cx,dy=event.clientY-cy;
    const raw=Math.hypot(dx,dy),scale=raw>max?max/raw:1; dx*=scale;dy*=scale;
    thumb.style.transform=`translate(${dx}px, ${dy}px)`; moveStick.classList.add('stick-active'); setMoveVector(dx/max,dy/max);
  });
  const endMove=event=>{ if(movePointer===null||(event.pointerId!=null&&event.pointerId!==movePointer))return; movePointer=null;thumb.style.transform='translate(0px, 0px)';moveStick.classList.remove('stick-active');clearMoveKeys(); };
  for(const type of ['pointerup','pointercancel','lostpointercapture']) moveStick.addEventListener(type,endMove);
}

bindStick(attackPad,{label:'ATAQUE',icon:'⚡',fireWhileHeld:true,onMove:(x,y,m)=>sendAim(x,y,m)});
